...
function SendEmail() {
  ...
  const sendEmail = () => {
    (async () => {
      try {
        emailjs.send('service_9b1piuw', 'template_31kxhvg', {from_name: "O'ktamboy" , reply_to: "bu juda ajoyib habar" ,message : "bizni saytdan foydalanganingiz uchun raxmat \n bu saytdan hali ancha vaqt foydalanasiz degan umiddamiz \n agar sayt tuzish kerak bo'lsa shu manzillarga murojat qiling \n telegram: https://t.me/OkTaM_bOy_0411", to_email: email , to_name: email }, 'KiJukHoeyLNnyatMO')
  ...
  return (
    <div className=" container flex items-center max-xl:flex-col max-xl:items-start max-xl:gap-5 justify-between py-12 bg-greenGrays-greenGray0.5">
        ...
        <div className=" bg-white rounded-full p-0 max-[570px]:bg-transparent">
            <input value={email} onChange={(e) => setEmail(e.target.value)} className=" outline-none p-4 px-8 bg-transparent text-gray-500 w-[320px] max-[570px]:bg-white max-[570px]:rounded-full max-[570px]:mb-5" type="text" placeholder="Your email address"/>
